...
exchanges_df = pd.read_csv('Crypto/data/exchanges2.csv')

for index, row in exchanges_df.iterrows():
    try:
        print(f"Processing {row['instrumentId']} on {row['exchange']}...")
        
        coin, exchange = row['instrumentId'], row['exchange']

        # Info
        urlSupportedCoins = f"https://open-api-v3.coinglass.com/api/futures/openInterest/ohlc-history?exchange={exchange}&symbol={coin}&interval=1d&limit=4500"
        urlSupportedExchanges = "https://open-api-v3.coinglass.com/api/futures/supported-exchange-pairs"

        # Open Interest
        urlOIOHLC = f"https://open-api-v3.coinglass.com/api/futures/openInterest/ohlc-history?exchange={exchange}&symbol={coin}&interval=1d&limit=4500"
        ...
        # Funding
        urlFunding = f"https://open-api-v3.coinglass.com/api/futures/fundingRate/ohlc-history?exchange={exchange}&symbol={coin}&interval=1d&limit=4500"
        # urlFundingHistoryOIWeight = f"https://open-api-v3.coinglass.com/api/futures/fundingRate/oi-weight-ohlc-history?symbol=BTC&interval=1d&limit=4500"
        # urlFundingHistoryVolWeight = f"https://open-api-v3.coinglass.com/api/futures/fundingRate/vol-weight-ohlc-history?symbol=BTC&interval=1d&limit=4500"

        # Liquidation
        urlLiquidation = f"https://open-api-v3.coinglass.com/api/futures/liquidation/history?exchange={exchange}&symbol={coin}&interval=1d&limit=4500"
        # urlLiquidationHistory = f"https://open-api-v3.coinglass.com/api/futures/liquidation/aggregated-history?symbol=BTC&interval=1d&limit=4500"
        ...
        # Long/Short
        urlGlobalLongShort = f"https://open-api-v3.coinglass.com/api/futures/globalLongShortAccountRatio/history?exchange={exchange}&symbol={coin}&interval=1d&limit=4500"
        urlAccountLongShort = f"https://open-api-v3.coinglass.com/api/futures/topLongShortAccountRatio/history?exchange={exchange}&symbol={coin}&interval=1d&limit=4500"
        urltopLongShortPositionRatio = f"https://open-api-v3.coinglass.com/api/futures/topLongShortPositionRatio/history?exchange={exchange}&symbol={coin}&interval=1d&limit=4500"
        urlaggregatedTakerBuySellVolumeRatio = f"https://open-api-v3.coinglass.com/api/futures/aggregatedTakerBuySellVolumeRatio/history?exchange={exchange}&symbol={coin}&interval=1d&limit=4500"
        urltakerBuySellVolume = f"https://open-api-v3.coinglass.com/api/futures/takerBuySellVolume/history?exchange={exchange}&symbol={coin}&interval=1d&limit=4500"
        ...
        dataframes = [oi_ohlc_data, funding_data, 
                    liquidation_history_data, global_long_short_data, 
        ...
        if not os.path.exists(f'Crypto/data/coinglass_{exchange}_{coin}.csv'):
            df_merged.to_csv(f'Crypto/data/coinglass_{exchange}_{coin}.csv', index=False)
        else:
            df_merged.to_csv(f'Crypto/data/coinglass_{exchange}_{coin}.csv', mode='a', header=False, index=False)
