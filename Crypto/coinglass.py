...
# Convert to DataFrame
ohlc_df = pd.DataFrame(ohlc_data, columns=['t', 'o', 'h', 'l', 'c', 'v'])
# Convert createTime from Unix time (seconds) to datetime
ohlc_df['t'] = pd.to_datetime(ohlc_df['t'], unit='s')
# drop o, h, l
ohlc_df = ohlc_df.drop(columns=['o', 'h', 'l'])
# Display the first few rows
print("PRICE:", ohlc_df.head())
...
# drop price
golden_ratio_data = golden_ratio_data.drop(columns=['price'])
# rename createTime to t
...
merged_df = pd.merge_asof(oi_data, ohlc_data, on='t', direction='nearest')
