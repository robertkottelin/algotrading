...
def prepare_market_data(df, suffix=''):
    # Define the columns you want to keep, including 'Volume'.
    columns_to_keep = {
        'Close': f"Close{suffix}",
        'Volume': f"Volume{suffix}"  # Keep the 'Volume' and rename to avoid confusion in the merged data.
    }
    # Select only the 'Close' and 'Volume' columns and rename them.
    df = df[['Close', 'Volume']].rename(columns=columns_to_keep)
    return df
...
# drop volume_vix
vix_data.drop(columns=['Volume_VIX'], inplace=True)
# rename close_vix to just vix
